class Show < ActiveRecord::Base
  ...
  scope :upcoming,     -> { where("date(performed_at) > ?", Time.zone.now.beginning_of_day).order(performed_at: :asc) }
  scope :latest,       -> { before_today.performed }
  scope :taped,        -> { where.not(embeds: "").ordered }
  scope :before_today, -> { where("date(performed_at) <= ?", Time.zone.now.beginning_of_day) }
